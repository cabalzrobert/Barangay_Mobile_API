            ...
            if (split.Length == 2)
            {
                ...
                if (!guser.IsEmpty() && !gpass.IsEmpty())
                {
                    ...
                    if (result != null)
                    {
                        ...
                        {
                            var result1 = _repo.DSpQueryMultiple("dbo.spfn_AEARP0A", new Dictionary<string, object>(){
                                { "parmplid", account.PL_ID },
                                { "parmpgrpid", account.PGRP_ID },
                                { "parmuserid", account.USR_ID },
...
<div style='margin: 10px' align='center'>
    <table cellspacing='0' cellpadding='0'>
        ...
        {
            var result = _repo.DSpQueryMultiple($"dbo.spfn_AEABDB0A", new Dictionary<string, object>()
            {
                {"parmplid",account.PL_ID },
                {"parmpgrpid",account.PGRP_ID },
