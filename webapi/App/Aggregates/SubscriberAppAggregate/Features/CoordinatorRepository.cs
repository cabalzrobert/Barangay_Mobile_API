...
namespace webapi.App.Aggregates.SubscriberAppAggregate.Features
{
    ...
    public interface ICoordinatorRepository
    {
        ...
        Task<(Results result, object item)> EventssAsync(FilterRequest filter);
        Task<(Results result, string message)> SaveEventsAsync(Events request, bool isUpdate = false);
    } 
    ...
    public class CoordinatorRepository : ICoordinatorRepository
    {
        ...
        public async Task<(Results result, object item)> EventssAsync(FilterRequest filter)
        {
            var results = _repo.DSpQueryMultiple($"dbo.spfn_0CCCBACBB02", new Dictionary<string, object>()
            {
                {"parmplid", account.PL_ID},
                {"parmpgrpid",account.PGRP_ID },
                { "parmsrch", filter.Search },
                { "parmeventdate", filter.BaseFilter },
            });
            if (results != null)
                return (Results.Success, SubscriberDto.EventNotification(account, results.Read<dynamic>(), 25));
            return (Results.Null, null);
        }
        ...
        public async Task<(Results result, string message)> SaveEventsAsync(Events request, bool isUpdate = false)
        {
            var result = _repo.DSpQuery<dynamic>("dbo.spfn_0CC01", new Dictionary<string, object>()
            {
                ...
                {"parmdate", request.EventDate},
                {"parmtime", request.EventTime},
                {"parmlocation", request.Location},
                {"parmprocessid", (isUpdate?request.ProccessID:"")},
                {"parmeventid", (isUpdate?request.EventID:"")},
            }).FirstOrDefault();
            if (result != null)
            {
                var row = ((IDictionary<string, object>)result);
                ...
                if (ResultCode == "1")
                {
                    ...
                    request.EventID = row["EVENT_ID"].Str();
                    ...
                    return (Results.Failed, "Events already exist");
            }
            return (Results.Null, null);
        }
