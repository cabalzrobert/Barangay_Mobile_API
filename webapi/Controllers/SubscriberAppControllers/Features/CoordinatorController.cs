...
namespace webapi.Controllers.SubscriberAppControllers.Features
{
    ...
    public class CoordinatorController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("events")]
        public async Task<IActionResult> Task0bca([FromBody] FilterRequest filter)
        {
            if (!FilterRequest.validity0da(filter)) return NotFound();
            var repoResult = await _coordRepo.EventssAsync(filter);
            if (repoResult.result == Results.Success)
        ...
        [HttpPost]
        [Route("event/new")]
        public async Task<IActionResult> Task0bcaa([FromBody] Events filter)
        {
            if (filter==null) return NotFound();
            var repoResult = await _coordRepo.SaveEventsAsync(filter);
            if (repoResult.result == Results.Success)
                return Ok(new { Status="ok", Message=repoResult.message, Content = filter});
        ...
        [HttpPost]
        [Route("event/edit")]
        public async Task<IActionResult> Task0bcab([FromBody] Events filter)
        {
            if (filter == null) return NotFound();
            var repoResult = await _coordRepo.SaveEventsAsync(filter,true);
            if (repoResult.result == Results.Success)
                return Ok(new { Status = "ok", Message = repoResult.message, Content = filter });
            if (repoResult.result == Results.Failed)
                return Ok(new { Status = "error", Message = repoResult.message });
